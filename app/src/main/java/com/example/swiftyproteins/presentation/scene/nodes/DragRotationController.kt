...
class DragRotationController(
    ...
    public override fun canStartTransformation(gesture: DragGesture): Boolean =
        transformableNode.isSelected

    private fun getX(lat: Double, long: Double): Float =
        (transformableNode.radius * cos(Math.toRadians(lat)) * sin(Math.toRadians(long))).toFloat()

    private fun getY(lat: Double, long: Double): Float =
        transformableNode.radius * sin(Math.toRadians(lat)).toFloat()

    private fun getZ(lat: Double, long: Double): Float =
        (transformableNode.radius * cos(Math.toRadians(lat)) * cos(Math.toRadians(long))).toFloat()
